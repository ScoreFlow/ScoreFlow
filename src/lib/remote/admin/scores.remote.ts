...
export const updateInstrument = form(
  updateInstrumentSchema,
  async ({ id, name }, invalid) => {
    const supabaseAdmin = await safeGetSupabaseServerAdmin();
    ...
    if (error) {
      invalid(
        "Het is niet gelukt om het instrument aan te passen. Probeer het later opnieuw.",
      );
      return;
    }
...
export const deleteInstrument = form(
  deleteInstrumentSchema,
  async ({ id }, invalid) => {
    const supabaseAdmin = await safeGetSupabaseServerAdmin();
    ...
    if (error) {
      invalid(
        "Het is niet gelukt om het instrument te verwijderen. Probeer het later opnieuw.",
      );
      return;
    }
