...
import { changePasswordSchema, loginSchema, oauthSchema, resetPasswordSchema } from "$lib/schemas/remote/auth"
...
export const resetPassword = form(resetPasswordSchema, async ({ email }, invalid) => {
  ...
  if (error) {
    console.error(error)
    invalid("Er is iets misgegaan bij het verzenden van de herstel-link. Probeer het later opnieuw.")
  }
...
export const changePassword = form(changePasswordSchema, async ({ password }, invalid) => {
  ...
  if (updateError) {
    if (updateError.code === "same_password") {
      invalid(invalid.password("Het nieuwe wachtwoord mag niet hetzelfde zijn als het oude wachtwoord"))
    } else {
