...
create table "public"."group_instrument_users"
(
    "id" bigint generated by default as identity not null,
    "instrument_id" uuid                                    not null,
    "group_id" uuid not null,
    "user_id" uuid not null,
    constraint "group_instrument_users_pkey" primary key ("id"),
    constraint "group_instrument_users_instrument_id_fkey" foreign key ("instrument_id") references "public"."instruments" ("id") on delete cascade,
    constraint "group_instrument_users_group_id_fkey" foreign key ("group_id") references "public"."groups" ("id") on delete cascade,
    constraint "group_instrument_users_user_id_fkey" foreign key ("user_id") references "auth"."users" ("id") on delete cascade
);
...
alter table "public"."group_instrument_users" enable row level security;
...
policy "Give access if user has access to instrument" on "public"."instruments"
    for
select using (id in (select instrument_id from public.group_instrument_users));

create
policy "Give access if user has access to instrument" on "public"."file_instruments"
    for
select using (instrument_id in (
    select group_instrument_users.instrument_id
    from public.group_instrument_users
    ));
...
create
policy "Give users access to their own data only" on "public"."group_instrument_users"
    for
